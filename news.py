...
article_count = st.slider("📊 目標成功抓取的總文章篇數：", min_value=0, max_value=25, value=6)

# ==================== 新增功能區塊 1：中英文新聞網站快速導航 ====================
with st.expander("🌐 中英文熱門新聞/知識網站快速參考（點擊開啟）"):
    ...
    with col_b:
        ...
        st.markdown("- [CNN](https://edition.cnn.com/)")
        st.markdown("- [Dogo News](https://www.dogonews.com/)")
        st.markdown("- [American Literature](https://americanliterature.com/)")
...
if st.button("🚀 步驟一：一鍵快速載入各領域穩定文章", type="primary"):
    if not selected_categories:
        ...
        if total_tasks == 0:
            ...
            if valid_articles:
                # 保留原本手動新增進來的文章，並將自動抓取的合併進去
                existing_manual = [a for a in st.session_state.articles_preview if a.get('category') == "手동自選" or a.get('category') == "手動自選"]
