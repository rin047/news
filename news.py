...
st.markdown("已完美整合**中英文切換、多執行緒秒抓、以及 GitHub 字型自動載入與備援**功能！")
st.markdown("---")
            ...
            with st.spinner("📄 正在排版並產生教材 PDF 中..."):
                try:
                    ...
                    if os.path.exists(font_path):
                        try:
                            pdfmetrics.registerFont(TTFont('CustomVariableFont', font_path))
                    ...
                    for _ in range(2):
                        ...
                        for art in export_list:
                            ...
                            story.append(Paragraph(f"領域：{art['category']} | 來源：{art['url']}", ParagraphStyle('Sub', parent=body_style, fontSize=8, textColor=colors.gray, fontName=font_name)))
