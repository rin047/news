...
st.markdown("已完美解決 GitHub 字型下載與 ReportLab 字型家族對應問題！")
st.markdown("---")
...
if st.button("🚀 步驟一：一鍵快速載入各領域穩定文章", type="primary"):
    if not selected_categories:
        ...
        if total_tasks == 0:
            ...
            if valid_articles:
                existing_manual = [a for a in st.session_state.articles_preview if a.get('category') == "手動自選"]
                combined = existing_manual + valid_articles
                for idx, art in enumerate(combined, 1):
                    art['index'] = idx
                st.session_state.articles_preview = combined
                st.success(f"✅ 成功載入文章，目前總計 {len(st.session_state.articles_preview)} 篇，請於下方檢查與勾選！")
            else:
                st.warning("⚠️ 載入發生阻擋，請重新點擊按鈕嘗試。")

# ==================== 中英文新聞網站快速導航 ====================
with st.expander("🌐 中英文熱門新聞/知識網站快速參考（點擊開啟）"):
    st.markdown("您可以從下方挑選喜歡的網站前往瀏覽，複製感興趣的文章網址：")
    col_a, col_b = st.columns(2)
    with col_a:
        st.markdown("**【中文新聞與知識網站】**")
        st.markdown("- [BBC 中文網](https://www.bbc.com/zhongwen/trad)")
        st.markdown("- [寰宇新聞網](https://www.globalnewstv.com.tw/)")
        st.markdown("- [PanSci 泛科學](https://pansci.asia/)")
        st.markdown("- [TechNews 科技新報](https://technews.tw/)")
        st.markdown("- [中央通訊社 (CNA)](https://www.cna.com.tw/)")
        st.markdown("- [風傳媒](https://www.storm.mg/)")
    with col_b:
        st.markdown("**【英文新聞與閱讀網站】**")
        st.markdown("- [BBC News](https://www.bbc.com/news)")
        st.markdown("- [News in Levels](https://www.newsinlevels.com/)")
        st.markdown("- [DOGO News](https://www.dogonews.com/)")
        st.markdown("- [Short Stories for Kids](https://www.shortstoriesforkids.net/)")
        st.markdown("- [National Geographic](https://www.nationalgeographic.com/)")
        st.markdown("- [Read Works](https://www.readworks.org/find-content#!g:/t:/pt:/features:/contentTab:search/)")

# ==================== 手動貼上多筆網址擷取 ====================
with st.expander("🔗 手動貼上文章網址自訂擷取（選填）"):
    manual_urls_input = st.text_area(
        "請在此貼上文章網址（每行一個網址）：",
        placeholder="https://example.com/article1\nhttps://example.com/article2",
        height=100
    )
    manual_category_tag = st.text_input("替手動貼上的文章自訂分類標籤：", value="手動自選")
    
    if st.button("➕ 抓取貼上的文章網址"):
        if not manual_urls_input.strip():
            st.warning("⚠️ 請先輸入至少一個網址！")
        else:
            urls_to_fetch = [u.strip() for u in manual_urls_input.split('\n') if u.strip()]
            added_count = 0
            
            with st.spinner("🔍 正在解析手動輸入的網址..."):
                for url in urls_to_fetch:
                    if any(a['url'] == url for a in st.session_state.articles_preview):
                        continue
                    
                    title, content, final_url = None, None, url
                    try:
                        headers = {
                            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36',
                        }
                        ...
                        soup = BeautifulSoup(html_content, 'html.parser')
                        if not title:
                            title = soup.title.string.strip() if soup.title else "無標題文章"
                        ...
                        if clean_text and len(clean_text) >= 50:
                            next_idx = len(st.session_state.articles_preview) + 1
                            st.session_state.articles_preview.append({
                                "index": next_idx,
                                "selected": True,
                                "category": manual_category_tag,
                                "url": final_url,
                                "title": title if title else "手動抓取文章",
                                "content": clean_text,
            ...
            if added_count > 0:
                st.success(f"✅ 成功手動新增並擷取了 {added_count} 篇文章！請至下方預覽區確認。")
            else:
                st.warning("⚠️ 無法成功解析您輸入的網址內容，請確認網址是否正確或具有存取權限。")
        ...
        if not selected_ones:
            ...
            with st.spinner("📄 正在排版並產生教材 PDF 中..."):
                try:
                    ...
                    if not os.path.exists(font_path):
                        res = requests.get(github_font_url, timeout=15)
                        if res.status_code == 200:
                            with open(font_path, "wb") as f:
                                f.write(res.content)
                    ...
                    if os.path.exists(font_path):
                        ...
                        st.error(f"❌ 找不到字型檔且無法從 GitHub 下載，請檢查網路連線！")
                        st.stop()
